...
sql = "CREATE TABLE IF NOT EXISTS Stations(" \
      "ID INTEGER UNIQUE AUTO_INCREMENT, name VARCHAR(50) UNIQUE NOT NULL , longitude DOUBLE , latitude DOUBLE , metro BIT, bus BIT,centeric BIT, PRIMARY KEY (ID)) DEFAULT CHARSET=utf8 DEFAULT COLLATE utf8_unicode_ci;"
...
for row in lines:
    elements = row.split(',')
    if len(elements) > 1:
        sql = "INSERT INTO Stations(name,longitude,latitude,metro,bus, centeric) VALUE('%s',%f,%f,%d,%d,%d)" % \
              (elements[0], float(elements[2]), float(elements[1]), 0, 1,0)
        print(sql)
        try:
            cursor.execute(sql)
